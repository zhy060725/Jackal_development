...
def main():
    rospy.init_node("semantic_cruise_controller")
    ...
    if not detector_kwargs.get("model_path"):
        rospy.logfatal(
            "semantic_cruise_controller requires model_path for RealSenseYOLODetector"
        )
    ...
    rospy.loginfo("semantic_cruise_controller publishing to %s", cmd_vel_topic)
        ...
        while not rospy.is_shutdown():
            try:
                ...
                rospy.loginfo_throttle(
                    1.0,
                    "semantic cruise reason=%s detections=%d linear.x=%.3f angular.z=%.3f",
                    command.reason,
                    len(detections),
                    command.linear_x,
                    command.angular_z,
                ...
                rospy.logerr_throttle(
                    1.0,
                    "YOLO capture or cruise planning failed; publishing stop: %s",
                    error,
                )
